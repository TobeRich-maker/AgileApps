...
import type { Task, CreateTaskRequest, ApiResponse, PaginatedResponse } from "../types/api"
...
export const tasksApi = {
  ...
  assign: async (id: string, assigneeId: string): Promise<Task> => {
    const response = await api.patch<ApiResponse<Task>>(`/tasks/${id}/assign`, { assigneeId })
    return response.data.data
  },
}
