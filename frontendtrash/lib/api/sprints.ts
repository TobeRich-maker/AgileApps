...
export const sprintsApi = {
  getAll: async (params?: {
    ...
    const response = await api.get<PaginatedResponse<Sprint>>("/sprints", { params })
  ...
  update: async (id: string, sprintData: Partial<CreateSprintRequest>): Promise<Sprint> => {
  ...
  complete: async (id: string): Promise<Sprint> => {
    const response = await api.post<ApiResponse<Sprint>>(`/sprints/${id}/complete`)
    return response.data.data
  },
}
