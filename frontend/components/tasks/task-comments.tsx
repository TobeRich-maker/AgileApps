...
const mockComments: Comment[] = [
  {
    id: "1",
    content:
      "I've started working on the authentication form. @jane-smith could you review the design mockups?",
    author: "John Doe",
    authorId: "user-1",
    ...
    replies: [
      {
        id: "1-1",
        content:
          "The mockups look great. Just one suggestion - can we add a forgot password link?",
        author: "Jane Smith",
        authorId: "user-2",
...
export function TaskComments({ taskId }: TaskCommentsProps) {
  ...
  const handleMentionSelect = (username: string) => {
    ...
    if (textarea) {
      ...
      if (lastAtIndex !== -1) {
        const newText =
          textBefore.substring(0, lastAtIndex) + `@${username} ` + textAfter;
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {comment.mentions.length > 0 && (
                      <Badge variant="secondary" className="text-xs">
                        {comment.mentions.length} mention
